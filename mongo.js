...
function Mongo(nWorker) {
  ...
  // secondaries (slaves), etc. The initial container is not necessarily the
  // primary.
  ...
  // "subsequent" containers are peers of the "initial" container. They receive
  // the starting replica set configuration from the initial container.
  ...
  // `PEERS` tells the "initial" container where to find the other members of
  // the replica set ("subsequent" containers).
  initialContainer.setEnv("PEERS", subsequentHosts.join(","));

  // All containers should be able to communicate in/out, as they could all
  // serve either role (primary, secondary), given a re-election upon failure,
  // etc.
